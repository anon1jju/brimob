...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login_submit'])) {
    ...
    if (file_exists($file_users)) {
        ...
        $users_data = is_array($users_data) ? $users_data : [];
        ...
        foreach ($users_data as $user) {
            ...
            if ($db_username === $input_username && $db_password === $input_password) {
                ...
                $_SESSION['nrp_petugas'] = $user['nrp'] ?? '';
                $_SESSION['pangkat_petugas'] = $user['pangkat'] ?? '';
    ...
    <div class="w-full rounded-lg bg-white shadow-md dark:border dark:border-gray-700 dark:bg-gray-800 sm:max-w-md md:mt-0 xl:p-0">
      <div class="space-y-4 p-6 sm:p-8 md:space-y-6">
        ...
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
            <span class="block sm:inline text-sm"><?= htmlspecialchars($error_msg) ?></span>
        </div>
        <?php endif; ?>
        ...
        <form method="POST" action="" class="space-y-4 md:space-y-6">
          ...
          <div>
            ...
            <input class="form-input block w-full rounded-md border border-gray-300 bg-gray-50 p-2.5 text-gray-900 focus:border-[var(--primary-600)] focus:ring-[var(--primary-600)] dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500 sm:text-sm"
                   id="username"
                   name="username"
          ...
          <button class="w-full rounded-md bg-[var(--primary-600)] px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-[var(--primary-700)] focus:outline-none focus:ring-4 focus:ring-[var(--primary-300)] dark:bg-[var(--primary-600)] dark:hover:bg-[var(--primary-700)] dark:focus:ring-[var(--primary-800)]"
                  type="submit">
            Sign in
          </button>
        </form>
